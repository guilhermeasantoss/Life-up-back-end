...
app.post("/criar/clientes", (req, res) => {
  ...
  con.query(sql, [nome, endereco, cpf, telefone, data_nascimento, email, senha], (err, result) => {
    ...
    enviarEmail({
      ...
      html: `<div style="font-family:'Poppins',Arial,sans-serif;background:#0f0f0f;padding:40px 20px;max-width:600px;margin:0 auto"><div style="background:#1a1a1a;border-radius:16px;overflow:hidden;border:1px solid #2a2a2a"><div style="background:linear-gradient(135deg,#1a0000,#2a0000);padding:32px;text-align:center;border-bottom:2px solid #e50914"><h1 style="color:#e50914;margin:0;font-size:2rem">⚡ LifeUp</h1><p style="color:#aaa;margin:8px 0 0;font-size:0.9rem">Suplementos</p></div><div style="padding:32px"><h2 style="color:#fff;margin:0 0 12px">Olá, ${nome}! 👋</h2><p style="color:#aaa;line-height:1.7;margin:0 0 20px">Seu cadastro na <strong style="color:#e50914">LifeUp Suplementos</strong> foi realizado com sucesso!</p><div style="background:#111;border-radius:10px;padding:20px;margin-bottom:24px;border:1px solid #2a2a2a"><p style="color:#888;font-size:0.8rem;margin:0 0 4px;text-transform:uppercase;letter-spacing:0.5px">Email cadastrado</p><p style="color:#fff;margin:0;font-size:0.95rem">${email}</p></div></div><div style="padding:20px 32px;border-top:1px solid #2a2a2a;text-align:center"><p style="color:#444;font-size:0.78rem;margin:0">© 2025 LifeUp Suplementos · support@lifeup.com</p></div></div></div>`
    });
...
app.put("/atualizar/clientes/:id", (req, res) => {
  ...
  const sql = `UPDATE clientes SET nome=?, email=?, data_nascimento=?, telefone=?, endereco=? WHERE id_clientes=?`;
...
app.post("/criar/pedidos", (req, res) => {
  ...
  con.query("INSERT INTO pedidos (id_clientes, valor_total) VALUES (?, ?)", [id_clientes, valor_total], (err, result) => {
    if (err) return res.status(500).json({ error: err.message });
    const idPedido = result.insertId;
    itens.forEach(item => {
      con.query("INSERT INTO itens_pedido (id_pedidos, id_produtos, quantidade, preco_unitario) VALUES (?,?,?,?)",
        [idPedido, item.id_produtos, item.quantidade, item.preco_unitario]);
      con.query("UPDATE produtos SET estoque = estoque - ? WHERE id_produtos = ? AND estoque >= ?",
        [item.quantidade, item.id_produtos, item.quantidade]);
    });
    con.query("SELECT nome, email FROM clientes WHERE id_clientes = ?", [id_clientes], (err2, rows) => {
      if (!err2 && rows.length > 0) {
        const { nome, email } = rows[0];
        const itensHTML = itens.map(i =>
          `<tr><td style="padding:10px 14px;color:#ccc;border-bottom:1px solid #222">${i.nome_produto || 'Produto #' + i.id_produtos}</td><td style="padding:10px 14px;color:#ccc;border-bottom:1px solid #222;text-align:center">${i.quantidade}</td><td style="padding:10px 14px;color:#e50914;border-bottom:1px solid #222;text-align:right;font-weight:600">R$ ${(i.preco_unitario * i.quantidade).toFixed(2).replace('.', ',')}</td></tr>`
        ).join('');
        enviarEmail({
          para: email,
          assunto: `Pedido #${idPedido} confirmado — LifeUp Suplementos`,
          html: `<div style="font-family:'Poppins',Arial,sans-serif;background:#0f0f0f;padding:40px 20px;max-width:600px;margin:0 auto"><div style="background:#1a1a1a;border-radius:16px;overflow:hidden;border:1px solid #2a2a2a"><div style="background:linear-gradient(135deg,#1a0000,#2a0000);padding:32px;text-align:center;border-bottom:2px solid #e50914"><h1 style="color:#e50914;margin:0;font-size:2rem">⚡ LifeUp</h1></div><div style="padding:32px"><h2 style="color:#fff;margin:0 0 6px">Pedido confirmado! 🎉</h2><p style="color:#aaa;margin:0 0 24px">Olá, <strong style="color:#fff">${nome}</strong>! Seu pedido foi recebido.</p><table style="width:100%;border-collapse:collapse;margin-bottom:20px"><thead><tr style="background:#111"><th style="padding:10px 14px;color:#666;font-size:0.78rem;text-align:left">Produto</th><th style="padding:10px 14px;color:#666;font-size:0.78rem;text-align:center">Qtd</th><th style="padding:10px 14px;color:#666;font-size:0.78rem;text-align:right">Total</th></tr></thead><tbody>${itensHTML}</tbody><tfoot><tr><td colspan="2" style="padding:14px;color:#fff;font-weight:700">Total</td><td style="padding:14px;color:#e50914;font-weight:700;text-align:right">R$ ${parseFloat(valor_total).toFixed(2).replace('.', ',')}</td></tr></tfoot></table></div><div style="padding:20px 32px;border-top:1px solid #2a2a2a;text-align:center"><p style="color:#444;font-size:0.78rem;margin:0">© 2025 LifeUp Suplementos</p></div></div></div>`
        });
      }
    });
...
app.get("/listar/pedidos/cliente/:id", (req, res) => {
  const id = parseInt(req.params.id, 10);
  if (isNaN(id)) return res.status(400).json({ error: "ID invalido" });
  const sql = `SELECT p.*, ip.id_produtos, ip.quantidade, ip.preco_unitario, pr.nome_produto
               FROM pedidos p
...
app.post("/criar/avaliacoes", (req, res) => {
  ...
  con.query("INSERT INTO avaliacoes (id_produtos, id_clientes, nota, comentario) VALUES (?, ?, ?, ?)",
    [id_produtos, id_clientes, nota, comentario], (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
      res.status(201).json({ id: result.insertId });
    });
});
...
app.put("/alterar/senha", (req, res) => {
  ...
  if (!email || !senha_atual || !senha_nova) return res.status(400).json({ error: "Dados obrigatorios faltando" });
  if (senha_nova.length < 6) return res.status(400).json({ error: "Nova senha deve ter no minimo 6 caracteres" });
  con.query("SELECT id_clientes FROM clientes WHERE email = ? AND senha = ?", [email, senha_atual], (err, result) => {
    if (err) return res.status(500).json({ error: err.message });
...
app.post("/verificar/identidade", (req, res) => {
  ...
  con.query("SELECT id_clientes FROM clientes WHERE email = ? AND cpf = ?", [email, cpf], (err, result) => {
    if (err) return res.status(500).json({ error: err.message });
...
app.put("/recuperar/senha", (req, res) => {
  ...
  if (!email || !cpf || !senha_nova) return res.status(400).json({ error: "Dados obrigatorios faltando" });
  if (senha_nova.length < 6) return res.status(400).json({ error: "Nova senha deve ter no minimo 6 caracteres" });
  con.query("UPDATE clientes SET senha = ? WHERE email = ? AND cpf = ?", [senha_nova, email, cpf], (err, result) => {
    if (err) return res.status(500).json({ error: err.message });
...
// PIX — Banco Inter
// =========================
con.query(`CREATE TABLE IF NOT EXISTS pagamentos_pix (
...
async function pixRecebidosInter(valor) {
  ...
  const resp   = await fetch(`${INTER.url}/pix/v2/pix?inicio=${inicio}&fim=${fim}`, {
    headers: { Authorization: `Bearer ${token}`, ...(INTER.conta ? { "x-conta-corrente": INTER.conta } : {}) },
    // @ts-ignore
    agent
  });
...
app.post("/pix/registrar", (req, res) => {
  ...
  con.query("INSERT INTO pagamentos_pix (txid, valor) VALUES (?, ?) ON DUPLICATE KEY UPDATE status='pendente', criado_em=NOW()",
    [txid, valor], (err) => {
      if (err) return res.status(500).json({ error: err.message });
...
app.get("/verificar/pix", async (req, res) => {
  ...
  if (INTER.client_id && fs.existsSync(INTER.cert) && fs.existsSync(INTER.key)) {
    try {
      ...
      if (pago) {
        con.query("UPDATE pagamentos_pix SET status='pago', pago_em=NOW() WHERE ABS(valor - ?) < 0.01 AND status='pendente'", [valor]);
...
app.post("/webhook/pix", (req, res) => {
  ...
  pixList.forEach(p => {
    ...
    if (txid) {
      ...
      con.query("UPDATE pagamentos_pix SET status='pago', pago_em=NOW() WHERE ABS(valor - ?) < 0.01 AND status='pendente' AND criado_em >= DATE_SUB(NOW(), INTERVAL 10 MINUTE) ORDER BY criado_em DESC LIMIT 1", [valor]);
    }
  });
...
app.post("/pix/confirmar-teste", (req, res) => {
  ...
  con.query("UPDATE pagamentos_pix SET status='pago', pago_em=NOW() WHERE ABS(valor - ?) < 0.01 AND status='pendente' ORDER BY criado_em DESC LIMIT 1",
    [parseFloat(valor)], (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
