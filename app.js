...
app.use(cors({
    origin: [
        "https://lifeup-suplementos.vercel.app",
        "https://life-up-chi.vercel.app"
    ],
    credentials: true
}));
app.use(morgan("dev"));
...
app.post("/criar/produtos", (req, res) => {
    ...
    const sql = `INSERT INTO produtos (nome_produto, descricao_produto, preco, marca, estoque, id_categoria, id_fornecedor) VALUES (?, ?, ?, ?, ?, ?, ?)`;
...
app.post("/criar/clientes", (req, res) => {
    ...
    const sql = `INSERT INTO clientes (nome, endereco, cpf, telefone, data_nascimento, email, senha) VALUES (?, ?, ?, ?, ?, ?, ?)`;
    con.query(sql, [nome, endereco, cpf, telefone, data_nascimento, email, senha], (err, result) => {
        ...
        enviarEmail({
            ...
            html: `<div style="font-family:'Poppins',Arial,sans-serif;background:#0f0f0f;padding:40px 20px;max-width:600px;margin:0 auto"><h1 style="color:#e50914">Bem-vindo, ${nome}!</h1><p style="color:#aaa">Seu cadastro foi realizado com sucesso na LifeUp Suplementos.</p></div>`
        });
...
app.post("/webhook/pix", (req, res) => {
    ...
    pixList.forEach(p => {
        ...
        if (txid) {
            ...
            con.query("UPDATE pagamentos_pix SET status='pago', pago_em=NOW() WHERE ABS(valor - ?) < 0.01 AND status='pendente' ORDER BY criado_em DESC LIMIT 1", [valor]);
        }
    });
