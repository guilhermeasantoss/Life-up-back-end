...
async function enviarEmail({ para, assunto, html }) {
    if (!EMAIL_CONFIG.auth.pass) { console.log(`[EMAIL SIMULADO] Para: ${para}`); return { ok: true }; }
    try { await transporter.sendMail({ from: `"LifeUp Suplementos" <${EMAIL_CONFIG.auth.user}>`, to: para, subject: assunto, html }); return { ok: true }; }
    catch (e) { console.error("Erro email:", e.message); return { ok: false }; }
}
...
app.post("/criar/produtos", (req, res) => {
    ...
    con.query("INSERT INTO produtos (nome_produto, descricao_produto, preco, marca, estoque, id_categoria, id_fornecedor) VALUES (?,?,?,?,?,?,?)",
...
app.put("/atualizar/produtos/:id", (req, res) => {
    ...
    con.query("UPDATE produtos SET nome_produto=?, descricao_produto=?, preco=?, marca=?, estoque=? WHERE id_produtos=?",
...
app.post("/criar/clientes", (req, res) => {
    ...
    con.query("INSERT INTO clientes (nome, endereco, cpf, telefone, data_nascimento, email, senha) VALUES (?,?,?,?,?,?,?)",
        ...
        (err, result) => {
            ...
            enviarEmail({ para: email, assunto: "Bem-vindo a LifeUp Suplementos!", html: `<h1>Bem-vindo, ${nome}!</h1><p>Seu cadastro foi realizado com sucesso.</p>` });
...
app.post("/login/clientes", (req, res) => {
    ...
    con.query("SELECT id_clientes, nome, email FROM clientes WHERE email = ? AND senha = ?", [email, senha], (err, result) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.put("/atualizar/clientes/:id", (req, res) => {
    ...
    con.query("UPDATE clientes SET nome=?, email=?, data_nascimento=?, telefone=?, endereco=? WHERE id_clientes=?",
...
app.get("/listar/pedidos/cliente/:id", (req, res) => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) return res.status(400).json({ error: "ID invalido" });
    con.query(`SELECT p.*, ip.id_produtos, ip.quantidade, ip.preco_unitario, pr.nome_produto
        FROM pedidos p
        LEFT JOIN itens_pedido ip ON p.id_pedidos = ip.id_pedidos
        LEFT JOIN produtos pr ON ip.id_produtos = pr.id_produtos
        WHERE p.id_clientes = ? ORDER BY p.id_pedidos DESC`, [id], (err, result) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(result);
    });
});
...
async function tokenInter() {
    ...
    const resp  = await fetch(`${INTER.url}/oauth/v2/token`, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body, agent });
...
app.post("/webhook/pix", (req, res) => {
    ...
    pixList.forEach(p => {
        if (p.txid) con.query("UPDATE pagamentos_pix SET status='pago', pago_em=NOW() WHERE txid = ?", [p.txid]);
        else con.query("UPDATE pagamentos_pix SET status='pago', pago_em=NOW() WHERE ABS(valor - ?) < 0.01 AND status='pendente' ORDER BY criado_em DESC LIMIT 1", [parseFloat(p.valor)]);
    });
